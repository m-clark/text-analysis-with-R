...
2. The following application of paste produces this result.

```{r ex_paste}
paste(c('A', '1', 'Q'), c('B', '2', 'z'))
```

How might you paste them so that there is an underscore `_` between characters and no space?

3. Use regex to grab the Star Wars names that have a number.
...
4. Use the <span class="pack">dplyr</span> helper functions  to grab all the columns in the <span class="objclass">starwars</span> data set with `color` in the name but without referring to them directly.  The following shows a generic example.  There are several ways to do this. Try two if you can. 
