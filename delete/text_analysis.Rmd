...
We demonstrate sentiment analysis with the text *The first thing the baby did wrong*, which is a very popular brief guide to parenting written by world renown psychologist Donald Barthelme who, in his spare time, also wrote postmodern literature.  This particular text talks about an issue with the baby, who likes to tear pages out of books. Attempts are made to rectify the situation, without much success, but things are finally resolved at the end.  The ultimate goal will be to see how sentiment in the text evolves over time.
...
Now that the data has been prepped, getting the sentiments is ridiculously easy.  But that is how it is with text analysis.  All the hard work is spent with the data processing.  Here all we need is an <span class="emph">inner join</span> of our words with a sentiment lexicon of choice. This process will only retain words that are also in the lexicon.  I use the numeric-based lexion here. At that point we get a sum score of sentiment by sentence.
...
Now we add the sentiments via the inner_join.  Here I use 'bing', but you can use another, and you might get a different result.
...
The following visualizes (via <span class="pack">plotly</span>) the positive and negative sentiment scores as one progresses sentence by sentence through the work.
...
In general it's a close game until the end, when negativity takes over as despair sets in with the story.
...
```{r barthelme_pos_vis, echo=FALSE}
carver_pos %>% 
  group_by(text) %>% 
  plot_ly() %>% 
...
One of the first things to do is interpret the topics, and we can start by seeing which terms are most probable for each topic.
...
For example, Hamlet is most likely to be associated with Topic `r t(topicmodels::topics(shakes_10, 3))['Hamlet',1]`. That topic is associated with the (stemmed words) `r topicmodels::get_terms(shakes_10, 20)[, t(topicmodels::topics(shakes_10, 3))['Hamlet',1]]`.  Sounds about right.
...
A couple things stand out.  To begin with, most works are associated with one topic[^toomanytopics].  In terms of the discovered topics, traditional classificiation really probably only works for the historical works, as they cluster together as expected (except for Henry the VIII, possibly due to it being a collaborative work).  Furthermore, tragedies and comedies might hit on the same topics, albeit from different perspecitves.  In addition, at least some works are very poetical, or at least have topics in common with the poems (love).  If we take four clusters, the result boils down to *Phoenix* (on its own), standard poems, a mixed bag of more love oriented works and the remaining poems, then everything else.  
...
```{r tm_chapter_cluster_topics, echo=FALSE, fig.align='center', fig.width=6, fig.height=7}
...
[^toomanytopics]: This may be partly due to the number of topics relative to row observations.
...
As another approach, consider the <span class="emph">saliency</span> and <span class="emph">relevance</span> of term via the <span class="pack">LDAvis</span> package.
...
t(topics(shakes_10, 2)) %>% 
  ...
  DT::datatable(options=list(dom='tp'), width='500') %>% 
  DT::formatStyle(
    0, # ignores columns, but otherwise put here
    target='row',
    backgroundColor = 'transparent'
...
For example, Hamlet is most associated with `r t(topicmodels::topics(shakes_10, 3))['Hamlet',1]`. That topic is associated with the (stemmed words) `r topicmodels::get_terms(shakes_10, 20)[, t(topicmodels::topics(shakes_10, 3))['Hamlet',1]]`.  Sounds about right.
...
# row_dend  = 
#   -quanteda::textstat_dist(dfm_weight(shakes_dtm, 'tfidf'), 
#                            margin = "documents", 
#                           method = "kullback") %>%
#   hclust(method="complete") %>%
#   as.dendrogram %>%
