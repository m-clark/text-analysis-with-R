...
fix_heatmaply = shakes_10@gamma %>%
  round(3) %>%
  heatmaply::heatmapr(Rowv=row_dend,
           Colv=F,
           labRow=shakes_10@documents,
           labCol=paste0('Topic ', 1:10),
           k_row = 4,
           row_side_colors = pull(arrange(shakes_types, title), class), # ignored by heatmaply
...
heatmaply::heatmaply(fix_heatmaply,
                     # labRow=shakes_10@documents,
                     colors='Oranges',
                     plot_method = 'plotly',
                     grid_gap=5,
                     ...
                     colorbar_len = 0.3,
                     colorbar_ypos=0) %>%
  # layout(showlegend=F, width=750, height=650) %>% # showing the legend will screw up the colorbar and any associated options
...
Given all these measures, one can now see how well they match what the topics the documents would be most associated with.  For example, Hamlet is most associated with `r t(topicmodels::topics(shakes_10, 3))['Hamlet',1]`.
