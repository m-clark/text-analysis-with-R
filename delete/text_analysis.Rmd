...
Let's start looking more in depth.  The following shows the 10 most common words and their respective counts. This is an easy way to find candidates to add to the stopword list.  Note that dai and prai are stems for day and pray. Love occurs `r round(top20[1]/top20[2], 2)` times as much as the most frequent word!
...
Alternatively, one could merely classify them based on their probable topics, which would make a more sense if classification is the goal. The following visualization attempts to order them based on their most probable topic.  The order are based on the most likely topics across all topics
