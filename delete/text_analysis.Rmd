...
We'll look at Shakespeare's plays and   We'll run run one with 10 topics.  We can also see how things compare with the usual classifications for the texts.  Also, this will take a while to run depending on your machine (maybe a minute or two).
...
Given all these measures, one can now see how well they match what the topics the documents would be most associated with.  For example, Hamlet is most associated with `r t(topics(shakes_10, 3))['Hamlet',1]`.
