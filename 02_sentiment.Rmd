...
baby = barth %>% 
  ...
  unnest_tokens(output=word, input=sentence, token='words', drop=F) %>% 
  ungroup 
```
...
baby_sentiment %>%
  mutate(running_avg = cumsum(score)/row_number()) %>% 
  plot_ly(width='50%') %>% 
  add_lines(x=~sentence_id, y=~running_avg,
            ...
            showlegend=T) %>%
  add_lines(x=~sentence_id, y=~score,
            color=I('#00aaff'), 
            line = list(shape = "spline"),
            showlegend=F) %>%
...
Looks like this one is going to be a downer. The following visualizes the positive and negative sentiment scores as one progresses sentence by sentence through the work. It uses the <span class="pack">plotly</span> package (note the theme is my own function, you'd need to supply your own).
