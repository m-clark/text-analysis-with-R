...
rnj %>% data.frame %>% head(500)  

rnj_filtered = rnj %>% 
  slice(-(1:49)) %>% 
  filter(!text == str_to_upper(text),
         !str_detect(text, pattern='^Scene'))
  
rnj_filtered %>% data.frame %>% head(500)  
rnj_filtered %>% 
  filter(ifelse(length(text)==1, T, F)) %$% 
  table(text)  

rnj_filtered2 = rnj_filtered %>% 
  unnest(text) %>% 
  unnest_tokens(text, input=text, token='sentences')
rnj_filtered2$text
```
