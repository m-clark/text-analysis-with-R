...
plot_data %>%
  ...
  ggplot(aes(x=line_chapter, y=negativity)) +
  geom_point(aes(y=mean_negativity*85, color=I(colspec)), alpha=.05, size=2) +
  # scale_color_viridis(option='C', discrete=T) +
  geom_line(aes(color=I(colspec), group=factor(chapter)), alpha=.9, lwd=.75) +
  # geom_point(aes(y=mean_negativity*85), stat='identity', color='#ff5500', alpha=.25) +
...
# 
plot_data %>% ungroup() %>%
  mutate(colspec = viridis::inferno(n=n_distinct(plot_data$chapter), begin=1, end=0)[chapter]) %>%
  plot_ly(colors=~rev(colspec), mode='lines') %>%
